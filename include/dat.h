...
class DatMgr {
 public:
  ...
  void print();
 private:
  std::unordered_map<std::string,Dot,std::hash<std::string>,std::equal_to<Dot>> dotlist;
  DatMgr() {
    if(debug) std::cout << "DatMgr private ctor\n";
    DatMgr::get_dots(dotlist);
  };

  void get_dots(std::unordered_map<std::string,Dot,std::hash<std::string>,std::equal_to<Dot>> &dots) {
